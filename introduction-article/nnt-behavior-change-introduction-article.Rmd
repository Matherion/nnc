...
ggsave(file.path(outputPath, "figure 1 - cer, d & nnc.pdf"), 
       width = 14,
       height = 8);
...
ggsave(file.path(outputPath, "figure 2 - d, cer & nnc.pdf"), 
       width = 14,
       height = 8);
...
```{r echo=TRUE}
  ...
  exampleData$behavior_dichotomous <-
    as.numeric(exampleData$behavior >= 135);
...
```{r echo=TRUE}
  ...
  behaviorchange::nnt(threshold=135,
                      mean = 90,
                      sd = 30,
                      d = .5);
