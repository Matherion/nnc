...
In this section, the examples from the paper will be shown, including the R commands used to generate the results. First, two hypothetical datasets will be generated that will be used for these tutorials.
...
```{r echo=TRUE}
  ...
  exampleData <- data.frame(behavior = rnorm(1000, mean=90, sd=30));

```

### Example 1: Continuous estimate

In this situation, we have a continuous behavior estimate, and we need to establish the CER. This means we first have to dichotomize this continuous measure. We can then establish the event rate and use this to compute the NNC.

The threshold value is 135 minutes, so this is where we dichotomize.
...
```{r echo=TRUE}
  ...
  freq(exampleData$behavior_dichotomous);
  
  ### Compute numbers needed for change
  nnc(cer=.07, mean = 90, sd = 30, d = .5);
...
We will again use the same data, with a mean of 90 minutes, a standard deviation of 30 minutes, a threshold of 135 minutes, and an effect size estimate of Cohen's *d* = 0.5.

In this situation, these numbers can just be directly provided to the `nnc` function.

```{r echo=TRUE}

  ### Compute numbers needed for change
  nnc(threshold=135, mean = 90, sd = 30, d = .5);
